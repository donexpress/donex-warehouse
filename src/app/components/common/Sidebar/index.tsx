...
const Index = () => {
    ...
    const getNavItemClass = (menu) => {
        let e = `/[locale]/${isOMS() ? 'oms' : ''}${isWMS() ? 'wms' : ''}/${menu.route}`
        return classNames(
            'flex items-center cursor-pointer nav-item-hover',
            {
        ...
        >
            <div className='flex flex-col'>
                <div className='flex  px-4 items-center justify-between relative'>
                    ...
                    <button onClick={() => { onToggleCollapse(true) }} className={collapseIconClass}>
                        {((width <= 991 && toggleCollapse) ) ?(<FaList />):(<FaAngleLeft/>)}
                        
                    </button>
                </div>
                <div className='flex flex-col items-start mt-4 scroll scrollable-hidden'>
                    <div className='group-nav flex flex-col items-start pb-3 cursor-pointer' key='HOME' onClick={()=>router.push(`/${locale}/${isOMS() ? 'oms' : ''}${isWMS() ? 'wms' : ''}`)}>
...

                    {
                        options.map((option, index) => (
                            <div className='group-nav flex flex-col items-start pb-3' key={index}>
                                <div className='flex py-3 px-4'>
                                    <div className='mr-1 text-xl flex items-center'>
                                        <Icons icons={option.icon} />
                                    </div>
                                    <span className='text-base font-medium text-sidebar'>{option.label}</span>
                                </div>
                                {                                   
                                    option.items?.map((op, i) => (
                                        <div className={getNavItemClass(op)} key={i + '-' + index}>
                                            {op.route ? (
                                                <a className='flex py-3  px-4' onClick={(e)=>{e.preventDefault(); router.push(`/${locale}/${isOMS() ? 'oms' : ''}${isWMS() ? 'wms' : ''}/${op.route}`)}} href={`/${locale}/${isOMS() ? 'oms' : ''}${isWMS() ? 'wms' : ''}/${op.route}`}>
                                                    {op.icon ? (
                                                        <>
                                                            <div className='mr-1 text-xl flex items-center'>
                                                                <Icons icons={op.icon} />
                                                            </div>
                                                            <div className='text-base font-medium text-sidebar'>{op.label}</div>
                                                        </>
                                                    ) : (
                                                        <div className='text-base font-medium text-sidebar ml-6'>{op.label}</div>
                                                    )}

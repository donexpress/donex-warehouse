...
import { packageShelfPath, autoAssignLocationPath } from '../backend';
...
export const autoAssignLocation = async (storagePlanId: number, values = {}): Promise<Response> => {
    const path = autoAssignLocationPath(storagePlanId);
    try {
      const response = await axios.post(path, values, getHeaders());
